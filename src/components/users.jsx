                        ...
                        <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,max-content))] gap-y-7 gap-x-2">
                            {
                             users.map((user) => (
                                    <div onClick={() => {navigate(`/profile/${user.id}`)}} key={user.id} className="shadow-lg rounded-lg overflow-hidden bg-gray-100 dark:bg-[#181818a3] pb-2">
                                        <img className="h-20 object-cover w-full" src={`https://socio-tech-server.onrender.com/uploads/${user.cover_photo}`} alt="" />
                                        <div className="px-2">
                                            <div className="mt-[-25px]">
                                                <img className="ml-2 h-[47px] w-[47px] object-cover rounded-full" src={`https://socio-tech-server.onrender.com/uploads/${user.profile_picture}`} alt="" />
